...
if __name__ == "__main__":
    ...
    agent = DQN_PERAgent(in_channels=state_channel,
                         action_num=action_dim, USE_CUDA=USE_CUDA, lr=learning_rate)
    ...
    for i in range(frames):
        ...
        episode_reward = reward1 + episode_reward
        agent.memory_buffer.push(frame, action, reward1, next_frame, done)
        frame = next_frame
        # print("RB:", env.printRB())
        loss = 0
        ...
        if i % print_interval == 0:
            print("frames: %5d, reward: %5f, loss: %4f, epsilon: %5f, episode: %4d" % (
                i, reward1, loss, epsilon, episode_num))  # np.mean(all_rewards[-10:])

            torch.save(agent.DQN.state_dict(), "./DQN_PER_dict.pth")
    ...
    torch.save(agent.DQN.state_dict(), "./DQN_PER_dict.pth.tar")
